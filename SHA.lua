...
function SHA.SHA1(input)
  local h = {[0]=0x67452301,0xEFCDAB89,0x98BADCFE,0x10325476,0xC3D2E1F0};
  input = SHA2_32_pad(input);
  local numblocks = (#input)/64;
  for a=0,(numblocks-1) do
    local w = {};
    local chunk = input:sub(a*64+1,(a+1)*64);
    for i=0,15 do
      w[i] = s2i(chunk,i*4);
    end
    for i=16,79 do
    ...
    for i=0,19 do
      f = bit32.bor(bit32.band(b,c),bit32.band(bit32.bnot(d),d));
      k = 0x5A827999;
      
      local temp = (bit32.lrotate(a,5)+f+e+k+w[i])%4294967296;
      e = d;
      d = c;
  ...
  return i2s(h[0])..i2s(h[1])..i2s(h[2])..i2s(h[3])..i2s(h[4]);
end
